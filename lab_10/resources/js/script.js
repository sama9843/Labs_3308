...
$(document).ready(function() {
  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    if(temp < 0) temp = 0;
    if(temp > 85) document.getElementById("thermometer_inner").style.background = "red";
    if(temp < 65) document.getElementById("thermometer_inner").style.background = "blue";
    if(temp < 85 && temp > 65) document.getElementById("thermometer_inner").style.background = "grey";
    ...
    for (var i = 0; i < 5; i++)
    {
      ...
      var day = getDay(i);
      var newcard = '<div class="card"><div class="card-body"><h5 class="card-title">' + day + '</h5><p class="card-text">High: ' + getFarenheitTemp(forecast.maxtemp)+ 'F' +
    '<br> Low: ' + getFarenheitTemp(forecast.mintemp) + 'F'+'<br>' +
        'Sunrise: ' + forecast.astro.sunrise + '<br>' +
        'Sunset: ' +  forecast.astro.sunset + '</p> </div> </div> </div>';
      $('#5_day_forecast').append(newcard);
    }
    /*
    ...
    */
    ...
    //helper funtion to return the day of the week
    function getDay(i){
      var week_names = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];
      dayOfWeek=week_names[new Date(Object.keys(data.forecast)[i]).getDay()];
      return dayOfWeek;
